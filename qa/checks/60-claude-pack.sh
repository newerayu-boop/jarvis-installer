...
# Шаблон намеренно без отрицающих скобок [^›]: в локали C многобайтовые
# символы внутри них сравниваются побайтно, и находка появлялась бы на одной
# машине и пропадала на другой.
while IFS= read -r doc; do
    [ -z "$doc" ] && continue
    case "$doc" in
        *_шаблон*|*шаблон*|*ШАБЛОН*) continue ;;
        QA.md|.claude/skills/auto-test/*) continue ;;   # документация самой QA-системы
    esac
    # Буквальное ‹…› — это описание метки в тексте («заполни все места ‹…›»),
    # а не забытая метка. Настоящая метка содержит слова: ‹сумма›, ‹имя›.
    n=$(grep '‹' "$PROJECT_ROOT/$doc" 2>/dev/null | grep -cvF '‹…›') || n=0
    [ "${n:-0}" -eq 0 ] && continue
    finding MINOR "$doc" "Строк с незаполненными местами: $n" \
