...
MUSIC = S / "demucs_out/htdemucs/v5_audio/no_vocals.wav"
OUT = S / "AIStrateg_kreo_v6.mp4"
...
i_20kun = inp(A/"stamp_20kun.png")
i_c1    = inp(A/"card1.png")
i_c2    = inp(A/"card2.png")
i_c3    = inp(A/"card3.png")
...
cur = "v10"
for n, (idx, t0, t1) in enumerate([(i_price, 12.10, 16.20),
                                   (i_4odam, 39.20, 43.40),
                                   (i_20kun, 47.55, 49.30)]):
    ov = stamp_chain(idx, f"st{n}", t0, t1, 1235)
    fc.append(f"[{cur}][st{n}]{ov}[vs{n}]")
    cur = f"vs{n}"

# --- нижние карточки (слайд вверх + фейд)
def card_chain(idx, label, t0, t1, y=1385):
    fc.append(f"[{idx}:v]format=rgba,fade=in:st={t0}:d=0.22:alpha=1,"
              f"fade=out:st={t1-0.25}:d=0.25:alpha=1[{label}]")
    return (f"overlay=x=(W-w)/2:y='{y}+26*(1-min(1,max(0,(t-{t0})/0.4)))'"
            f":enable='between(t,{t0},{t1})'")

for n, (idx, t0, t1) in enumerate([(i_c1, 16.40, 24.20),
                                   (i_c2, 31.50, 39.00),
                                   (i_c3, 49.40, 55.30)]):
    ov = card_chain(idx, f"cd{n}", t0, t1)
    fc.append(f"[{cur}][cd{n}]{ov}[vc{n}]")
    cur = f"vc{n}"
...
fc.append(f"[{i_music}:a]volume=1.0,apad=pad_dur=4,atrim=0:{T}[mus0]")
